...
public class CashClient{
    ...
    public class RPCListener implements Runnable{
        ...
        public void run() {
            ...
            StreamObserver<CashRequest> cash = stub.dealCash(new StreamObserver<CashReply>() {
                @Override
                public void onNext(CashReply cashReply) {
            ...
            cash.onNext(CashRequest.newBuilder().setUser("ziapple").setMoney(1).build());
            System.out.println("发送给服务器消息user:leilei,money:2");
            cash.onNext(CashRequest.newBuilder().setUser("leilei").setMoney(2).build());
